...
export default function LoginPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved && expire && Date.now() < Number(expire)) {
        ...
        if (WEAK_PINS.includes(user.pin)) {
  ...
  async function handleLogin() {
    ...
    const expire = Date.now() + (keepLogin ? KEEP_DAYS : 1) * 24 * 60 * 60 * 1000
      ...
      <div style={{ width: '100%', maxWidth: 360, padding: '0 24px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 20, padding: 28, border: '1px solid #E8ECF0', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input value={nm} onChange={e => { setNm(e.target.value); setError('') }} placeholder="이름을 입력하세요" style={inp} />
          </div>

          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#888', marginBottom: 6 }}>PIN</div>
            <input type="password" placeholder="4자리" maxLength={4} value={pin}
              onChange={e => { setPin(e.target.value); setError('') }}
              onKeyDown={e => e.key === 'Enter' && handleLogin()}
              style={{ ...inp, textAlign: 'center', letterSpacing: 12, fontSize: 22 }} />
          </div>

          <div onClick={() => setKeepLogin(v => !v)}
            ...
            <span style={{ fontSize: 13, color: '#666', fontWeight: 500 }}>로그인 유지 ({KEEP_DAYS}일)</span>
          </div>
