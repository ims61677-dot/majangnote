...
function getNextPayday(): { date: string; label: string } {
  ...
  let payMonth = day <= 15 ? month : month + 1
  let payYear = year
...
function ProgressSteps({ item }: { item: any }) {
  const steps = [
    { label: '신청 완료', sub: timeAgo(item.created_at), done: true, color: '#00B894' },
    {
      label: '검토 중',
      sub: item.status === 'approved' ? '승인 완료 ✓' : item.status === 'rejected' ? '반려됨' : item.status === 'manager_approved' ? '대표 검토 중' : '검토 대기',
      done: item.status === 'approved', color: item.status === 'approved' ? '#6C5CE7' : '#bbb',
      current: item.status === 'pending' || item.status === 'manager_approved',
    },
    {
      label: item.repaid ? '상환 완료' : '최종 완료',
      sub: item.status === 'approved' ? (item.repaid ? '상환 완료 ✓' : '미상환') : item.status === 'rejected' ? '반려됨' : '대기 중',
      done: item.status === 'approved' && item.repaid, color: item.repaid ? '#00B894' : item.status === 'approved' ? '#FDC400' : '#bbb',
      current: item.status === 'approved' && !item.repaid,
    },
  ]
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', marginBottom: 12, marginTop: 4 }}>
      {steps.map((step, i) => (
        <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
          {i < steps.length - 1 && <div style={{ position: 'absolute', top: 11, left: '50%', width: '100%', height: 2, background: steps[i+1].done || steps[i+1].current ? step.color : '#E8ECF0', zIndex: 0 }} />}
          <div style={{ width: 22, height: 22, borderRadius: '50%', zIndex: 1, flexShrink: 0, background: step.done ? step.color : step.current ? '#fff' : '#F4F6F9', border: step.current ? '2px solid #FF6B35' : step.done ? 'none' : '2px solid #E8ECF0', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: step.current ? '0 0 0 3px rgba(255,107,53,0.15)' : 'none' }}>
            {step.done ? <span style={{ fontSize: 11, color: '#fff', fontWeight: 800 }}>✓</span> : step.current ? <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#FF6B35', display: 'block' }} /> : <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#ddd', display: 'block' }} />}
          </div>
...
const AGREEMENTS = [
  { id: 'a1', text: '본 신청은 당월 또는 차월 급여의 선지급에 한하며, 퇴직금 선지급은 관련 법규 및 사내 규정에 따릅니다.' },
  { id: 'a2', text: (payLabel: string) => `상환 기한은 다가오는 급여일(${payLabel})이며, 해당 기한까지 무이자입니다. 기한 초과 시 잔여 금액은 연 12% 이자율의 대출로 자동 전환됨에 동의합니다.` },
  { id: 'a3', text: '선지급금 및 발생 이자는 급여 지급 시 우선 차감될 수 있으며, 이에 동의합니다.' },
  { id: 'a4', text: '재직 중 미상환 잔액이 있는 상태에서 무단퇴사 또는 임의 퇴직 시, 잔여 금액은 즉시 연 20% 이자율의 대출로 전환되며, 퇴직금 및 미지급 급여에서 우선 차감될 수 있음에 동의합니다.' },
...
function ApplyForm({ storeId, userName, userId, onSaved, onClose }: {
  ...
  async function checkAttendance() {
    ...
    try {
      ...
      const { data } = await supabase.from('attendance').select('status, work_date').eq('profile_id', userId).gte('work_date', monthStart).lte('work_date', monthEnd)
      const absents = (data||[]).filter((a:any) => a.status === 'absent')
      const lates = (data||[]).filter((a:any) => a.status === 'late' || a.status === 'late_early')
      if (absents.length > 0) { setBlockReason(`이번 달 결근 ${absents.length}회가 있어 선입금 신청이 불가해요.`); setOnDuty(false) }
      else if (lates.length > 0) { setBlockReason(`이번 달 지각 ${lates.length}회가 있어 선입금 신청이 불가해요.`); setOnDuty(false) }
      else { setOnDuty(true) }
  ...
  async function handleSubmit() {
    setError('')
    if (!amount || isNaN(Number(amount.replace(/,/g,''))) || Number(amount.replace(/,/g,'')) <= 0) { setError('신청 금액을 올바르게 입력해주세요'); return }
    ...
    try {
      const { data: req } = await supabase.from('advance_requests').insert({
        ...
        amount: Number(amount.replace(/,/g,'')), reason, status: 'pending', payday: payday.date,
      }).select().single()
      if (req) {
        const { data: managers } = await supabase.from('store_members').select('profiles(nm)').eq('store_id', storeId).in('role', ['owner','manager']).eq('active', true)
        if (managers && managers.length > 0) {
          await supabase.from('advance_notifications').insert(managers.map((m:any) => ({ request_id: req.id, store_id: storeId, target_name: m.profiles?.nm||'', type: 'new_request' })))
        }
      }
      onSaved()
  ...
  function formatAmount(val: string) { const num = val.replace(/[^0-9]/g,''); return num ? Number(num).toLocaleString() : '' }
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
      <div style={{ width:'100%', maxWidth:480, background:'#F4F6F9', borderRadius:'20px 20px 0 0', padding:'20px 16px 40px', maxHeight:'92vh', overflowY:'auto' }}>
        ...
        {checking ? <div style={{ textAlign:'center', padding:40, color:'#bbb' }}>출퇴근 기록 확인 중...</div>
        : !onDuty ? (
          ...
          <>
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <div style={{ position:'relative' }}>
                <input value={amount} onChange={e => setAmount(formatAmount(e.target.value))} placeholder="0" style={{ ...inp, paddingRight:30, textAlign:'right', fontSize:16, fontWeight:700 }} />
            ...
            <div style={{ ...bx, border:'1px solid rgba(108,92,231,0.2)', background:'rgba(108,92,231,0.02)', marginBottom:14 }}>
              ...
              {AGREEMENTS.map(ag => {
                ...
                return (
                  <div key={ag.id} onClick={() => setAgreed(p => ({ ...p, [ag.id]: !p[ag.id] }))} style={{ display:'flex', gap:10, alignItems:'flex-start', padding:'10px 12px', borderRadius:10, marginBottom:6, border:`1px solid ${isChecked?'rgba(108,92,231,0.3)':'#E8ECF0'}`, background:isChecked?'rgba(108,92,231,0.04)':'#F8F9FB', cursor:'pointer' }}>
                    <div style={{ width:18, height:18, borderRadius:5, flexShrink:0, marginTop:1, border:`2px solid ${isChecked?'#6C5CE7':'#D0D5DD'}`, background:isChecked?'#6C5CE7':'#fff', display:'flex', alignItems:'center', justifyContent:'center' }}>
                      {isChecked && <span style={{ color:'#fff', fontSize:11, fontWeight:800 }}>✓</span>}
                    </div>
                    <span style={{ fontSize:11, color:isChecked?'#6C5CE7':'#555', lineHeight:1.7, fontWeight:isChecked?600:400 }}>{text}</span>
                  </div>
                )
              })}
              <div onClick={() => { const nv=!allAgreed; const all:Record<string,boolean>={}; AGREEMENTS.forEach(a=>{all[a.id]=nv}); setAgreed(all) }} style={{ display:'flex', gap:10, alignItems:'center', padding:'10px 12px', borderRadius:10, marginTop:4, border:`1px solid ${allAgreed?'rgba(255,107,53,0.4)':'#E8ECF0'}`, background:allAgreed?'rgba(255,107,53,0.06)':'#F4F6F9', cursor:'pointer' }}>
                <div style={{ width:18, height:18, borderRadius:5, flexShrink:0, border:`2px solid ${allAgreed?'#FF6B35':'#D0D5DD'}`, background:allAgreed?'#FF6B35':'#fff', display:'flex', alignItems:'center', justifyContent:'center' }}>
            ...
            <button onClick={handleSubmit} disabled={saving||reason.length<200||!amount||!allAgreed}
              style={{ width:'100%', padding:'13px 0', borderRadius:12, border:'none', color:'#fff', fontSize:14, fontWeight:700, background:(saving||reason.length<200||!amount||!allAgreed)?'#ddd':'linear-gradient(135deg,#FF6B35,#E84393)', cursor:(saving||reason.length<200||!amount||!allAgreed)?'not-allowed':'pointer' }}>
              {saving?'제출 중...':!allAgreed?'동의 항목을 모두 체크해주세요':'신청서 제출 (검토 요청)'}
            </button>
          </>
        )}
      </div>
...
function ManagerApproveModal({ item, managerName, onDone, onClose }: {
  ...
  async function handleApprove() {
    ...
    setSaving(true)
    try {
      await supabase.from('advance_requests').update({ status:'manager_approved', manager_approved_by:managerName, manager_approved_at:new Date().toISOString(), manager_comment:comment }).eq('id', item.id)
      const { data: owners } = await supabase.from('store_members').select('profiles(nm)').eq('store_id', item.store_id).eq('role','owner').eq('active',true)
      if (owners && owners.length > 0) await supabase.from('advance_notifications').insert(owners.map((o:any) => ({ request_id:item.id, store_id:item.store_id, target_name:o.profiles?.nm||'', type:'manager_approved' })))
      onDone()
    } finally { setSaving(false) }
  }
  ...
  async function handleReject() {
    ...
    setSaving(true)
    try {
      await supabase.from('advance_requests').update({ status:'rejected', rejected_by:managerName, rejected_at:new Date().toISOString(), reject_reason:rejectReason }).eq('id', item.id)
      await supabase.from('advance_notifications').insert({ request_id:item.id, store_id:item.store_id, target_name:item.author_name, type:'rejected' })
      onDone()
    } finally { setSaving(false) }
  }
    ...
    <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
      <div style={{ width:'100%', maxWidth:480, background:'#F4F6F9', borderRadius:'20px 20px 0 0', padding:'20px 16px 40px', maxHeight:'92vh', overflowY:'auto' }}
        ...
        <div style={{ ...bx, border:'1px solid rgba(255,107,53,0.2)' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}><span style={{ fontSize:13, color:'#888' }}>신청자</span><span style={{ fontSize:13, fontWeight:700 }}>{item.author_name}</span></div>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}><span style={{ fontSize:13, color:'#888' }}>금액</span><span style={{ fontSize:15, fontWeight:800, color:'#FF6B35' }}>{Number(item.amount).toLocaleString()}원</span></div>
          {item.payday && <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10 }}><span style={{ fontSize:13, color:'#888' }}>무이자 상환 기한</span><span style={{ fontSize:13, fontWeight:700, color:'#00B894' }}>{item.payday.replace(/-/g,'.')} (급여일)</span></div>}
        ...
        <div style={{ background:'rgba(232,67,147,0.04)', border:'1px solid rgba(232,67,147,0.2)', borderRadius:12, padding:'10px 14px', marginBottom:12 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#E84393', marginBottom:4 }}>📋 차용증 효력 동의 완료</div>
          <div style={{ fontSize:10, color:'#888', lineHeight:1.7 }}>급여일 무이자 · 초과 연 12% · 무단퇴사 연 20% · 급여/퇴직금 차감 동의</div>
        </div>
        <div style={{ background:'rgba(108,92,231,0.05)', border:'1px solid rgba(108,92,231,0.15)', borderRadius:12, padding:'12px 14px', marginBottom:12 }}>
          ...
          <div style={{ fontSize:11, color:'#888', lineHeight:1.8 }}>• 끝까지 스크롤해야 승인 버튼 활성화<br/>• 승인 의견 <strong style={{ color:'#1a1a2e' }}>100자 이상</strong> 필수<br/>• 승인 시 <strong style={{ color:'#1a1a2e' }}>대표님께 자동 보고</strong><br/>• 허위 승인 시 관리자 책임 귀속</div>
        </div>
        {!scrolled && <div style={{ textAlign:'center', fontSize:11, color:'#FF6B35', fontWeight:700, marginBottom:10, padding:'8px', background:'rgba(255,107,53,0.06)', borderRadius:8 }}>↓ 끝까지 스크롤해야 승인 버튼이 활성화됩니다</div>}
        <div style={{ marginBottom:12 }}>
          ...
          <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="신청자와 면담한 내용, 승인 사유 등을 100자 이상 작성해주세요." rows={4} style={{ ...inp, resize:'none', lineHeight:1.7 }} />
        </div>
        <div style={{ marginBottom:14 }}>
          {[
...
function OwnerApproveModal({ item, ownerName, onDone, onClose }: {
  ...
  async function handleApprove() {
    ...
    try {
      await supabase.from('advance_requests').update({
        status:'approved', owner_approved_by:ownerName, owner_approved_at:new Date().toISOString(),
        manager_approved_by: item.manager_approved_by || ownerName,
        manager_approved_at: item.manager_approved_at || new Date().toISOString(),
      }).eq('id', item.id)
      await supabase.from('advance_notifications').insert({ request_id:item.id, store_id:item.store_id, target_name:item.author_name, type:'approved' })
      onDone()
    } finally { setSaving(false) }
  }

  async function handleReject() {
    if (rejectReason.length < 2) { alert('반려 사유를 입력해주세요'); return }
    if (!confirm('반려 처리하시겠습니까?')) return
    setSaving(true)
    try {
      await supabase.from('advance_requests').update({ status:'rejected', rejected_by:ownerName, rejected_at:new Date().toISOString(), reject_reason:rejectReason }).eq('id', item.id)
      await supabase.from('advance_notifications').insert({ request_id:item.id, store_id:item.store_id, target_name:item.author_name, type:'rejected' })
      onDone()
    } finally { setSaving(false) }
  }
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:200, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'flex-end', justifyContent:'center' }}>
      <div style={{ width:'100%', maxWidth:480, background:'#F4F6F9', borderRadius:'20px 20px 0 0', padding:'20px 16px 40px', maxHeight:'92vh', overflowY:'auto' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          <span style={{ fontSize:15, fontWeight:700, color:'#1a1a2e' }}>👑 선입금 검토</span>
          <button onClick={onClose} style={{ background:'none', border:'none', fontSize:20, color:'#bbb', cursor:'pointer' }}>✕</button>
        </div>
        {item.manager_approved_by && (
          <div style={{ ...bx, border:'1px solid rgba(108,92,231,0.2)', background:'rgba(108,92,231,0.02)', marginBottom:12 }}>
            <div style={{ fontSize:11, fontWeight:700, color:'#6C5CE7', marginBottom:6 }}>✓ 관리자 검토 완료</div>
            <div style={{ fontSize:12, color:'#666' }}>승인자: <strong>{item.manager_approved_by}</strong></div>
            {item.manager_comment && <div style={{ fontSize:12, color:'#666', marginTop:4, lineHeight:1.6 }}>의견: {item.manager_comment}</div>}
          </div>
        )}
        <div style={{ ...bx, border:'1px solid rgba(255,107,53,0.2)' }}>
          <div style={{ fontSize:11, color:'#aaa', marginBottom:8, fontWeight:700 }}>신청 내용</div>
        ...
        <div style={{ background:'rgba(232,67,147,0.04)', border:'1px solid rgba(232,67,147,0.2)', borderRadius:12, padding:'10px 14px', marginBottom:16 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#E84393', marginBottom:4 }}>📋 차용증 효력 동의 완료</div>
          <div style={{ fontSize:10, color:'#888', lineHeight:1.7 }}>급여일({item.payday?.replace(/-/g,'.')||'-'}) 무이자 · 초과 연 12% · 무단퇴사 연 20% · 급여/퇴직금 차감 동의</div>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => setMode(mode==='reject'?'view':'reject')} style={{ flex:1, padding:'13px 0', borderRadius:12, background:'rgba(232,67,147,0.08)', border:'1px solid rgba(232,67,147,0.25)', color:'#E84393', fontSize:14, fontWeight:700, cursor:'pointer' }}>반려</button>
          <button onClick={handleApprove} disabled={saving} style={{ flex:2, padding:'13px 0', borderRadius:12, background:'linear-gradient(135deg,#FF6B35,#E84393)', border:'none', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer' }}>
            {saving?'처리 중...':'✓ 승인'}
          </button>
        </div>
        {mode === 'reject' && (
          <div style={{ marginTop:12, padding:14, background:'#fff', borderRadius:12, border:'1px solid rgba(232,67,147,0.2)' }}>
            <div style={{ fontSize:12, fontWeight:700, color:'#E84393', marginBottom:8 }}>반려 사유</div>
            <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)} placeholder="반려 사유를 입력해주세요." rows={3} style={{ ...inp, resize:'none', marginBottom:8 }} />
            <button onClick={handleReject} disabled={saving} style={{ width:'100%', padding:'10px 0', borderRadius:10, background:'#E84393', border:'none', color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer' }}>반려 처리</button>
          </div>
        )}
      </div>
...
export default function AdvancePage() {
  ...
  async function loadData(sid: string, uname: string, role: string, uid: string) {
    setLoading(true)
    try {
      const { data } = await supabase.from('advance_requests').select('*').eq('store_id', sid).order('created_at', { ascending:false })
      setItems(data||[])
      if (role === 'owner') {
        const { data: memberData } = await supabase.from('store_members').select('store_id, stores(id, name)').eq('profile_id', uid).eq('role','owner').eq('active',true)
        if (memberData && memberData.length > 0) {
          setMyStores(memberData.map((m:any) => m.stores))
          const sm:Record<string,string> = {}
          memberData.forEach((m:any) => { sm[m.stores.id]=m.stores.name })
          setStoreMap(sm)
          const { data: allData } = await supabase.from('advance_requests').select('*').in('store_id', memberData.map((m:any) => m.store_id)).order('created_at', { ascending:false })
          setAllItems(allData||[])
        }
      }
  ...
  async function handleDelete(id: string) {
    if (!confirm('이 신청 건을 삭제하시겠습니까?')) return
    await supabase.from('advance_requests').delete().eq('id', id)
    loadData(storeId, userName, userRole, userId)
  }

  async function handleRepaid(id: string) {
    if (!confirm('상환 완료 처리하시겠습니까?')) return
    await supabase.from('advance_requests').update({ repaid: true, repaid_at: new Date().toISOString() }).eq('id', id)
    loadData(storeId, userName, userRole, userId)
  }

  async function handleUnrepaid(id: string) {
    if (!confirm('미상환으로 변경하시겠습니까?')) return
    await supabase.from('advance_requests').update({ repaid: false, repaid_at: null }).eq('id', id)
    loadData(storeId, userName, userRole, userId)
  }
  ...
  const filtered = isManager
    ? baseItems.filter((i:any) => {
        if (filterStatus==='all') return true
        if (filterStatus==='repaid') return i.status==='approved' && i.repaid
        if (filterStatus==='unrepaid') return i.status==='approved' && !i.repaid
        return i.status === filterStatus
      })
    : myItems
  const pendingCount = (isOwner && viewTab==='all'?allItems:items).filter((i:any) => i.status==='pending'||i.status==='manager_approved').length
  const managerPendingCount = items.filter((i:any) => i.status==='pending').length
  ...
  const tabStyle = (active: boolean): React.CSSProperties => ({
    ...
    borderColor:active?'#1a1a2e':'#E8ECF0', background:active?'#1a1a2e':'#F8F9FB', color:active?'#fff':'#888',
  })
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          {isOwner && pendingCount>0 && <span style={{ fontSize:10, padding:'2px 7px', borderRadius:10, background:'#FF6B35', color:'#fff', fontWeight:700 }}>검토 {pendingCount}</span>}
          {isManager && !isOwner && managerPendingCount>0 && <span style={{ fontSize:10, padding:'2px 7px', borderRadius:10, background:'#6C5CE7', color:'#fff', fontWeight:700 }}>검토 {managerPendingCount}</span>}
        </div>
        {isStaff && <button onClick={() => setShowApply(true)} style={{ padding:'6px 14px', borderRadius:9, background:'rgba(255,107,53,0.1)', border:'1px solid rgba(255,107,53,0.3)', color:'#FF6B35', fontSize:12, fontWeight:700, cursor:'pointer' }}>+ 신청하기</button>}
      </div>
      ...
      {isOwner && myStores.length > 1 && (
        <div style={{ display:'flex', background:'#F4F6F9', borderRadius:12, padding:4, marginBottom:14 }}>
          ...
          <button onClick={() => setViewTab('all')} style={{ flex:1, padding:'10px 0', borderRadius:10, border:'none', cursor:'pointer', fontSize:13, fontWeight:viewTab==='all'?700:400, background:viewTab==='all'?'#fff':'transparent', color:viewTab==='all'?'#1a1a2e':'#aaa', boxShadow:viewTab==='all'?'0 1px 4px rgba(0,0,0,0.08)':'none' }}>
            🏢 전체 지점 {pendingCount>0?`(검토 ${pendingCount})`:''}
          </button>
        </div>
      )}
      ...
      {isManager && (
        <div style={{ display:'flex', gap:5, marginBottom:12, flexWrap:'wrap' }}>
          {[
            ...
            { val:'approved', label:`승인 ${baseItems.filter((i:any)=>i.status==='approved').length}` },
            { val:'repaid', label:`상환완료 ${baseItems.filter((i:any)=>i.status==='approved'&&i.repaid).length}` },
            { val:'unrepaid', label:`미상환 ${baseItems.filter((i:any)=>i.status==='approved'&&!i.repaid).length}` },
            { val:'rejected', label:`반려 ${baseItems.filter((i:any)=>i.status==='rejected').length}` },
          ].map(opt => <button key={opt.val} onClick={() => setFilterStatus(opt.val)} style={tabStyle(filterStatus===opt.val)}>{opt.label}</button>)}
        </div>
      )}
      ...
      {loading ? (
        ...
        const canOwnerApprove = isOwner && (item.status==='pending'||item.status==='manager_approved')
        return (
          <div key={item.id} style={{ ...bx, borderLeft:item.repaid?'3px solid #00B894':item.status==='pending'?'3px solid #FF6B35':item.status==='manager_approved'?'3px solid #6C5CE7':item.status==='approved'?'3px solid #FDC400':'3px solid #E84393' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
              <div>
                {isManager && (
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:2 }}>
                    <span style={{ fontSize:12, color:'#FF6B35', fontWeight:700 }}>{item.author_name}</span>
                    {isOwner && viewTab==='all' && storeMap[item.store_id] && <span style={{ fontSize:10, padding:'1px 6px', borderRadius:6, fontWeight:600, background:'rgba(108,92,231,0.1)', color:'#6C5CE7' }}>📍 {storeMap[item.store_id]}</span>}
                  </div>
                )}
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  {item.status==='approved' && (
                    item.repaid
                      ? <span style={{ fontSize:10, padding:'2px 7px', borderRadius:6, background:'rgba(0,184,148,0.1)', color:'#00B894', fontWeight:700 }}>✓ 상환완료</span>
                      : <span style={{ fontSize:10, padding:'2px 7px', borderRadius:6, background:'rgba(253,196,0,0.15)', color:'#B8860B', fontWeight:700 }}>⚠️ 미상환</span>
                  )}
                </div>
              ...
              {isOwner && <button onClick={() => handleDelete(item.id)} style={{ fontSize:11, color:'#E84393', background:'rgba(232,67,147,0.08)', border:'1px solid rgba(232,67,147,0.2)', borderRadius:7, padding:'4px 10px', cursor:'pointer', fontWeight:600, flexShrink:0 }}>삭제</button>}
            </div>
            ...
            {canOwnerApprove && <button onClick={() => setApproveItem(item)} style={{ width:'100%', padding:'10px 0', borderRadius:10, cursor:'pointer', background:'linear-gradient(135deg,#FF6B35,#E84393)', border:'none', color:'#fff', fontSize:13, fontWeight:700, marginBottom:6 }}>👑 승인/반려 처리</button>}

            {isOwner && item.status==='approved' && (
              item.repaid
                ? <button onClick={() => handleUnrepaid(item.id)} style={{ width:'100%', padding:'9px 0', borderRadius:10, cursor:'pointer', background:'#F4F6F9', border:'1px solid #E8ECF0', color:'#aaa', fontSize:12, fontWeight:600 }}>미상환으로 변경</button>
                : <button onClick={() => handleRepaid(item.id)} style={{ width:'100%', padding:'9px 0', borderRadius:10, cursor:'pointer', background:'rgba(0,184,148,0.1)', border:'1px solid rgba(0,184,148,0.3)', color:'#00B894', fontSize:13, fontWeight:700 }}>✓ 상환 완료 처리</button>
            )}
          </div>
        )
      })}
