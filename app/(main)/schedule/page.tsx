...
function GridEditor({ year, month, schedules, staffList, isManager, storeId, myName, onSaved, onChangeMonth }: {
  ...
  const syncScroll = useCallback((sourceLeft: number) => {
    if (isSyncing.current) return
    isSyncing.current = true
    if (headerScrollRef.current) headerScrollRef.current.scrollLeft = sourceLeft
    bodyScrollRefs.current.forEach(ref => { if (ref) ref.scrollLeft = sourceLeft })
    if (footerScrollRef.current) footerScrollRef.current.scrollLeft = sourceLeft
  ...
  useEffect(() => {
    ...
    const offset = Math.max(0, (todayDay - 3) * 44)
    setTimeout(() => {
      syncScroll(offset)
    }, 100)
  ...
  async function handleSave(status: string, position: string, note: string) {
    ...
    await supabase.from('schedules').upsert({
      store_id: storeId,
      staff_name: popup.staff,
      schedule_date: popup.date,
      status,
  ...
  if (visibleStaff.length === 0) {
    return (
      <div style={{ background:'#fff', borderRadius:16, border:'1px solid #E8ECF0', padding:32, textAlign:'center' }}>
        ...
        <div style={{ fontSize:11, color:'#ddd', marginTop:4 }}>직원관리에서 직원을 먼저 등록해주세요</div>
      </div>
    )
  }
            ...
            {days.map(day => {
              ...
              return (
                <div key={day} style={{ minWidth:44, flexShrink:0, padding:'6px 2px', textAlign:'center',
                  background: isToday ? 'rgba(108,92,231,0.07)' : '#F8F9FB',
          ...
          <div key={staff} style={{ display:'flex', borderTop: si > 0 ? '1px solid #F0F2F5' : 'none' }}>
            ...
            <div ref={el => { bodyScrollRefs.current[si] = el }}
              ...
              {days.map(day => {
                ...
                return (
                  <div key={day}
                    onClick={() => isManager && setPopup({ staff, date: dateStr })}
...
export default function SchedulePage() {
  ...
  return (
    <div>
      ...
      {viewTab==='grid' && (
        <GridEditor
          year={calYear} month={calMonth} schedules={schedules} staffList={staffList}
          isManager={isManager} storeId={storeId} myName={myName}
          onSaved={() => loadData(storeId, calYear, calMonth)}
          onChangeMonth={handleChangeMonth} />
      )}
      {viewTab==='month' && (
        <MonthlyView
          year={calYear} month={calMonth} schedules={schedules}
