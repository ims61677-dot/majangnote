      ...
      {statsLoading ? (
        ...
        <>
          ...
          <div style={{ background:'#fff', borderRadius:14, padding:'14px', marginBottom:12, border:'1px solid #F0F0F0' }}>
            <div style={{ fontSize:13, fontWeight:700, marginBottom:12, color:'#1a1a2e' }}>🏪 지점별 완료 현황</div>
            {stores.map((store: any) => {
              const storeTodos = statsData.allTodos.filter((t: any) => t.store?.id === store.id)
              const storeDone = storeTodos.filter((t: any) => t.checkers.length > 0).length
              const storeTotal = storeTodos.length
              const rate = storeTotal > 0 ? Math.round((storeDone / storeTotal) * 100) : 0
              const storeChecks = Object.values(statsData.personMap as Record<string,any>)
                .reduce((sum: number, v: any) => {
                  // personMap은 전체라 지점 분리 안됨 - allTodos의 checkers로 집계
                  return sum
                }, 0)
              const storeCheckCount = storeTodos.reduce((sum: number, t: any) => sum + t.checkers.length, 0)
              return (
                <div key={store.id} style={{ marginBottom:14 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:5 }}>
                    <span style={{ fontSize:13, fontWeight:700, color:'#1a1a2e' }}>{store.name}</span>
                    <div style={{ display:'flex', gap:10, fontSize:11, color:'#888' }}>
                      <span>할일 {storeTotal}개</span>
                      <span style={{ color:'#00B894', fontWeight:600 }}>완료 {storeDone}건</span>
                      <span style={{ color:'#6C5CE7', fontWeight:600 }}>체크 {storeCheckCount}건</span>
                      {storeTotal - storeDone > 0 && <span style={{ color:'#E84393', fontWeight:600 }}>미완료 {storeTotal - storeDone}건</span>}
                    </div>
                  </div>
                  <div style={{ height:8, borderRadius:4, background:'#F4F6F9' }}>
                    <div style={{ height:8, borderRadius:4, background:'linear-gradient(90deg,#6C5CE7,#00B894)', width:`${rate}%`, transition:'width 0.4s' }} />
                  </div>
                  <div style={{ fontSize:10, color:'#aaa', marginTop:2, textAlign:'right' }}>{rate}% 완료</div>
                </div>
              )
            })}
            {stores.length === 0 && <div style={{ textAlign:'center', color:'#ccc', fontSize:12 }}>지점 정보 없음</div>}
          </div>
          <div style={{ background:'#fff', borderRadius:14, padding:'14px', marginBottom:12, border:'1px solid #F0F0F0' }}>
