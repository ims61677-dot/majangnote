...
export default function RecipePage() {
    ...
    const result = await Promise.all(
      members.map(async (m: any) => {
        ...
        const [{ data: recData }, { data: catData }] = await Promise.all([
          supabase.from('recipes').select('*').eq('store_id', s.id).order('created_at'),
  ...
  // 드래그 앤 드롭 순서 변경 (대분류)
  async function handleDropTop(dragId: string, dropId: string) {
  ...
  function openEdit(r: any) {
    setEditRec(r); setNm(r.name); setParentCat(r.category || ''); setSubCat(r.sub_category || ''); setIc(r.icon)
  ...
  // ── 레시피 그룹핑 (대분류→소분류) ────────────
  const filteredRec = filterCat === '전체' ? recipes : recipes.filter(r => r.category === filterCat)
  type Group = { parentName: string; subs: { subName: string | null; recs: any[] }[] }

  function buildGroups(): Group[] {
    const parentNames = Array.from(new Set(filteredRec.map(r => r.category))).sort((a, b) => {
      const ai = topCats.findIndex(c => c.name === a)
      const bi = topCats.findIndex(c => c.name === b)
      return (ai < 0 ? 999 : ai) - (bi < 0 ? 999 : bi)
    })
    return parentNames.map(pName => {
      const pRecs    = filteredRec.filter(r => r.category === pName)
      ...
      if (children.length > 0) {
        const noSub = pRecs.filter(r => !r.sub_category)
        if (noSub.length > 0) subs.push({ subName: null, recs: noSub })
        children.forEach(ch => {
          const sr = pRecs.filter(r => r.sub_category === ch.name)
        ...
        subs.push({ subName: null, recs: pRecs })
      }
  ...
  function openAdminEdit(recipe: any, sid: string) {
    ...
    setANm(recipe.name); setAParent(recipe.category || ''); setASub(recipe.sub_category || ''); setAIc(recipe.icon)
  ...
  async function executeCopyAll() {
    ...
    const [{ data: recData }, { data: catData }] = await Promise.all([
      supabase.from('recipes').select('*').eq('store_id', copyAllTarget).order('created_at'),
  ...
  // ── 레시피 카드 ───────────────────────────────
  function RecipeCard({ r, onEdit, onDelete }: { r: any; onEdit?: () => void; onDelete?: () => void }) {
    const isHl = highlight === r.id
    return (
      <div ref={el => { itemRefs.current[r.id] = el }}
        style={{ ...bx, border: isHl ? '2px solid #FF6B35' : '1px solid #E8ECF0', background: isHl ? '#FFF8F5' : '#ffffff', transition: 'border 0.3s, background 0.3s' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}
          ...
          {(onEdit || onDelete) && (
            <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
              {onEdit   && <button onClick={e => { e.stopPropagation(); onEdit() }}   style={{ background: 'none', border: 'none', color: '#bbb', cursor: 'pointer', fontSize: 12 }}>✏️</button>}
              {onDelete && <button onClick={e => { e.stopPropagation(); onDelete() }} style={{ background: 'none', border: 'none', color: '#bbb', cursor: 'pointer', fontSize: 11 }}>✕</button>}
            </div>
          )}
    ...
    <div>
      ...
      {activeTab === 'mine' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
            <button onClick={() => { setFilterCat('전체'); setHighlight(null) }}
              style={{ padding: '5px 12px', borderRadius: 20, fontSize: 11, cursor: 'pointer',
                ...
                color: filterCat === '전체' ? '#FF6B35' : '#888' }}>전체</button>
            {topCats.map(c => (
              <button key={c.id} onClick={() => { setFilterCat(c.name); setHighlight(null) }}
                style={{ padding: '5px 12px', borderRadius: 20, fontSize: 11, cursor: 'pointer',
                  ...
                  color: filterCat === c.name ? '#FF6B35' : '#888' }}>
                {c.name}
              </button>
            ))}
          </div>
          ...
          {groups.map(group => (
            <div key={group.parentName} style={{ marginBottom: 24 }}>
              {/* 대분류 헤더 */}
              <div style={{ fontSize: 13, fontWeight: 800, color: '#FF6B35', marginBottom: 10, padding: '5px 12px', background: 'rgba(255,107,53,0.06)', borderRadius: 8, display: 'inline-block' }}>
                {group.parentName}
              </div>
              {group.subs.map(sub => (
                <div key={sub.subName || '_'} style={{ marginBottom: 10 }}>
                  {/* 소분류 헤더 */}
                  {sub.subName && (
                    <div style={{ fontSize: 11, fontWeight: 700, color: '#2DC6D6', marginBottom: 6, marginLeft: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ color: '#ddd' }}>└</span> {sub.subName}
                    </div>
                  )}
                  {sub.recs.map(r => (
              ...
              <div key={store.id} style={{ ...bx, padding: 0, overflow: 'hidden' }}>
                ...
                <div onClick={() => setExpandedStore(expandedStore === store.id ? null : store.id)}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#aaa' }}>레시피 {sr.length}개 · 카테고리 {(sCats || []).filter((c: Cat) => !c.parent_id).length}개</div>
                  </div>
                        ...
                        <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 0', borderBottom: '1px solid #F8F9FB' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 10, color: '#bbb' }}>
                              {r.category}{r.sub_category ? ` > ${r.sub_category}` : ''} · 재료 {r.ingredients.length}가지
                            </div>
                          </div>
                          <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
        ...
        <div style={overlay} onClick={() => setShowCatMgr(false)}>
          <div style={{ ...mBox, maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            ...
            {topCats.map(cat => (
              <div key={cat.id}
                draggable
                onDragStart={() => { setDragCatId(cat.id); setDragOverId(null) }}
                onDragOver={e => { e.preventDefault(); setDragOverId(cat.id) }}
                onDrop={() => { if (dragCatId) handleDropTop(dragCatId, cat.id); setDragCatId(null); setDragOverId(null) }}
                ...
                <div style={{
                  ...
                  background: dragOverId === cat.id ? '#FFF0EB' : '#F8F9FB',
                  border: `1px solid ${dragOverId === cat.id ? '#FF6B35' : '#E8ECF0'}`,
                  ...
                  <button onClick={() => { setNewSubParent(newSubParent === cat.id ? '' : cat.id); setNewSubName('') }}
                    style={{ padding: '3px 8px', borderRadius: 6, background: 'rgba(45,198,214,0.1)', border: '1px solid rgba(45,198,214,0.3)', color: '#2DC6D6', fontSize: 10, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                    + 소분류
                  </button>
                ...
                {newSubParent === cat.id && (
                  <div style={{ display: 'flex', gap: 6, marginTop: 6, marginLeft: 16 }}>
                    <input value={newSubName} onChange={e => setNewSubName(e.target.value)}
                      ...
                      style={{ ...inp, flex: 1, fontSize: 12 }} autoFocus />
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8, marginBottom: 12 }}>
              <input value={newCatName} onChange={e => setNewCatName(e.target.value)}
                ...
                placeholder="새 대분류 이름 (예: 파스타, 피자...)"
                style={{ ...inp, flex: 1 }} />
              <button onClick={addTopCat}
                style={{ padding: '8px 14px', borderRadius: 8, background: 'rgba(255,107,53,0.1)', border: '1px solid rgba(255,107,53,0.3)', color: '#FF6B35', fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                + 추가
              </button>
            </div>
            <button onClick={() => setShowCatMgr(false)}
              style={{ width: '100%', padding: 10, borderRadius: 8, background: '#F4F6F9', border: '1px solid #E8ECF0', color: '#888', cursor: 'pointer', fontSize: 13 }}>
              닫기
            </button>
          </div>
        </div>
          ...
          <div style={mBox} onClick={e => e.stopPropagation()}>
            {copyAllStep === 'select' && (
              <>
                ...
                <div style={{ fontSize: 12, color: '#aaa', marginBottom: 16 }}>"{copyAllModal.sourceName}" 레시피+카테고리를 어느 지점으로 복사할까요?</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            ...
            {commonStep === 'form' && (
              <>
                ...
                <div style={{ marginBottom: 8 }}>
                  <input value={cParent} onChange={e => { setCParent(e.target.value); setCSub('') }} placeholder="대분류 이름 직접 입력" style={inp} />
                  <div style={{ fontSize: 10, color: '#bbb', marginTop: 4, marginLeft: 2 }}>※ 전 지점 공통 등록은 카테고리를 직접 입력해주세요</div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={checkCommonDup} disabled={!cNm.trim() || !cParent.trim()}
                    style={{ flex: 1, padding: 10, borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 700, cursor: (cNm.trim() && cParent.trim()) ? 'pointer' : 'default', background: (cNm.trim() && cParent.trim()) ? 'linear-gradient(135deg,#FF6B35,#E84393)' : '#E8ECF0', color: (cNm.trim() && cParent.trim()) ? '#fff' : '#bbb' }}>
                    중복 확인 →
                  </button>
