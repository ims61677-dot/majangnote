                  ...
                  {filtered.map(item => {
                    ...
                    return (
                      <div key={item.id} data-stock-id={item.id} style={getCardStyle(item.id, tot, item.min_qty, wq, { margin: 0, ...(isHL ? { border: '2px solid #6C5CE7', background: 'rgba(108,92,231,0.04)' } : {}) })}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isEdit ? 6 : 0 }}>
                          <button onClick={() => setShowQuickOrder(item)}
                            style={{ padding: '5px 12px', borderRadius: 8, background: 'linear-gradient(135deg,#FF6B35,#E84393)', border: 'none', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                            📋 발주요청
                          </button>
                        </div>
                        {isEdit && (
