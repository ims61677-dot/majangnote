...
function EntryModal({ sheet, entry, storeId, userName, year, month, favorites, onSave, onClose }: {
  ...
  async function handleSave() {
    ...
    // 자주쓰는 품목 자동 저장/카운트 증가
    if (itemName.trim()) {
      const fav = favorites.find(f => f.name === itemName.trim())
      if (fav) {
        await supabase.from('settlement_favorites').update({
          use_count: (fav.use_count||0)+1,
          default_amount: Number(amount),
          default_payment: payment
        }).eq('id', fav.id)
      } else {
        try {
          await supabase.from('settlement_favorites').insert({
            store_id: storeId, sheet_id: sheet.id,
            name: itemName.trim(), default_amount: Number(amount),
            default_payment: payment, use_count: 1
          })
        } catch {}
      }
    }
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:300, display:'flex', alignItems:'flex-end', justifyContent:'center' }} onClick={onClose}>
      <div style={{ background:'#fff', width:'100%', maxWidth:480, borderRadius:'20px 20px 0 0', padding:20, maxHeight:'92vh', overflowY:'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              {favorites.slice(0,8).map(f => (
                <button key={f.id} onClick={() => {
                  setItemName(f.name)
                  if (f.default_amount) setAmount(f.default_amount)
                  if (f.default_payment) setPayment(f.default_payment)
        ...
        {opt.deposit_date && (
          <div style={{ marginBottom:10 }}>
            <span style={lbl}>입금일자</span>
            <input type="date" value={depositDate} onChange={e => setDepositDate(e.target.value)} style={inp} />
          </div>
        )}
        <div style={{ marginBottom:10 }}>
          <span style={lbl}>품목명 (선택)</span>
          <input value={itemName} onChange={e => setItemName(e.target.value)} placeholder={`예: ${sheet.name} 구매`} style={inp} />
        </div>
...
function SheetManageModal({ sheets, storeId, onSave, onClose }: {
  ...
  async function handleAdd() {
    ...
    await supabase.from('settlement_sheets').insert({
      store_id: storeId, name: newName.trim(), icon: newIcon,
      sheet_type: 'expense', sort_order: maxOrder+1
    })
  ...
  async function handleToggleField(sheet: any, field: string) {
    ...
    await supabase.from('settlement_sheets').update({
      optional_fields: { ...current, [field]: !current[field] }
    }).eq('id', sheet.id)
    onSave()
  }
        ...
        <div style={{ background:'rgba(255,107,53,0.04)', borderRadius:12, padding:14, border:'1px dashed rgba(255,107,53,0.3)', marginTop:8 }}>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
              {ICONS.map(ic => (
                <button key={ic} onClick={() => setNewIcon(ic)} style={{ width:34, height:34, borderRadius:8, border: newIcon===ic?'2px solid #FF6B35':'1px solid #E8ECF0', background: newIcon===ic?'rgba(255,107,53,0.1)':'#F8F9FB', fontSize:17, cursor:'pointer' }}>{ic}</button>
              ))}
            </div>
          </div>
...
function ProfitAnalysisView({ sheets, storeId, year, month, settings }: {
  ...
  function getFeeForPlatform(keywords: string[]) {
    return feeEntries.filter(e => keywords.some(kw => e.item_name?.includes(kw))).reduce((s, e) => s + (e.amount||0), 0)
  }
  ...
  return (
    <div>
      ...
      <div style={bx}>
        ...
        {[
          { name:'배달의민족', sales:baemin, fee:baeminFee, icon:'🛵' },
          { name:'쿠팡이츠',   sales:coupang, fee:coupangFee, icon:'🟡' },
          { name:'요기요',     sales:yogiyo, fee:yogiyoFee, icon:'🔴' },
        ].filter(p => p.sales > 0 || p.fee > 0).map(p => (
            ...
            {p.fee > 0 && (
              <>
                ...
                <div style={{ display:'flex', justifyContent:'flex-end' }}>
                  <span style={{ fontSize:10, color:'#aaa' }}>순수령: </span>
                  <span style={{ fontSize:11, fontWeight:700, color:'#6C5CE7', marginLeft:4 }}>{numFmt(p.sales - p.fee)}원</span>
                </div>
              </>
            )}
          </div>
        ...
        {totalSales === 0 && (
          <div style={{ textAlign:'center', padding:'16px 0', color:'#bbb', fontSize:12 }}>마감일지에서 매출 입력 시 자동 연동됩니다</div>
        )}
        {totalSales > 0 && (
          <div style={{ borderTop:'2px dashed #E8ECF0', paddingTop:10, marginTop:8 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
              <span style={{ fontSize:12, color:'#555' }}>총 수수료</span>
              <span style={{ fontSize:12, fontWeight:700, color:'#E84393' }}>-{numFmt(totalFee)}원 ({pct(totalFee, totalSales)}%)</span>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between' }}>
              <span style={{ fontSize:13, fontWeight:700, color:'#1a1a2e' }}>수수료 차감 후 실수령</span>
              <span style={{ fontSize:15, fontWeight:800, color:'#6C5CE7' }}>{numFmt(netSales)}원</span>
            </div>
          </div>
        )}
      </div>
            ...
            <div key={sheet.id} style={{ marginBottom: amt>0?14:6 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: amt>0?4:0 }}>
                ...
                <div style={{ textAlign:'right' }}>
                  {amt>0 ? (
                    <>
                      <span style={{ fontSize:13, fontWeight:700, color:'#1a1a2e' }}>{numFmt(amt)}원</span>
                      {totalSales>0 && <span style={{ fontSize:10, color:'#aaa', marginLeft:5 }}>매출대비 {ratio}%</span>}
                    </>
      ...
      {totalExpense > 0 && (
        <div style={bx}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:14 }}>
            {[
              ...
              <div key={item.l} style={{ padding:'12px', background:'#F8F9FB', borderRadius:12, border:`1px solid ${item.warn&&totalSales>0?'rgba(232,67,147,0.3)':'#E8ECF0'}` }}>
                ...
                {totalSales>0 && (
                  <>
                    <div style={{ fontSize:11, fontWeight:600, color: item.warn?'#E84393':'#aaa' }}>매출 대비 {item.pv}%</div>
                    <div style={{ fontSize:9, color:'#bbb', marginTop:2 }}>{item.note}</div>
                  </>
                )}
              </div>
            ))}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {topExpense && (
              <div style={{ padding:'10px 14px', background:'rgba(108,92,231,0.07)', borderRadius:10, border:'1px solid rgba(108,92,231,0.2)' }}>
                <span style={{ fontSize:12, fontWeight:700, color:'#6C5CE7' }}>💡 최대 지출 항목</span>
                <span style={{ fontSize:12, color:'#555', marginLeft:8 }}>{topExpense.icon} {topExpense.name} — {numFmt(entrySums[topExpense.id]||0)}원 ({pct(entrySums[topExpense.id]||0, totalSales)}%)</span>
              </div>
            )}
            {totalSales>0 && pct(materialTotal,totalSales)>35 && (
              <div style={{ padding:'10px 14px', background:'rgba(232,67,147,0.07)', borderRadius:10, border:'1px solid rgba(232,67,147,0.2)' }}>
                <span style={{ fontSize:12, fontWeight:700, color:'#E84393' }}>⚠️ 재료비 비중이 {pct(materialTotal,totalSales)}%로 높아요</span>
                <div style={{ fontSize:11, color:'#aaa', marginTop:2 }}>재료비는 매출의 30~35% 이하 유지를 권장해요</div>
              </div>
            )}
            {profitRate < 10 && totalSales > 0 && (
              <div style={{ padding:'10px 14px', background:'rgba(253,196,0,0.1)', borderRadius:10, border:'1px solid rgba(253,196,0,0.3)' }}>
                <span style={{ fontSize:12, fontWeight:700, color:'#B8860B' }}>⚠️ 수익률이 {profitRate}%로 낮아요</span>
                <div style={{ fontSize:11, color:'#aaa', marginTop:2 }}>지출 중 줄일 수 있는 항목이 있는지 확인해보세요</div>
              </div>
            )}
            {expenseSheets.filter(s => !entrySums[s.id]).length > 0 && (
              <div style={{ padding:'10px 14px', background:'#F8F9FB', borderRadius:10, border:'1px solid #E8ECF0' }}>
                <span style={{ fontSize:12, fontWeight:700, color:'#aaa' }}>📋 미입력 시트</span>
                <span style={{ fontSize:11, color:'#bbb', marginLeft:6 }}>{expenseSheets.filter(s => !entrySums[s.id]).map(s => s.name).join(', ')}</span>
              </div>
            )}
          </div>
        </div>
...
function SheetView({ sheet, storeId, userName, year, month }: {
  ...
  async function loadFavorites() {
    const { data } = await supabase.from('settlement_favorites').select('*')
      .eq('sheet_id', sheet.id).order('use_count', { ascending: false })
    setFavorites(data || [])
  }

  async function deleteFavorite(id: string) {
    await supabase.from('settlement_favorites').delete().eq('id', id)
    loadFavorites()
  }
    ...
    <div>
      <div style={{ ...bx, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button onClick={() => { setEditEntry(null); setShowModal(true) }}
          style={{ padding:'12px 18px', borderRadius:12, background:'linear-gradient(135deg,#FF6B35,#E84393)', border:'none', color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer' }}>
          + 항목 추가
        </button>
      </div>

      {favorites.length > 0 && (
        <div style={{ ...bx, paddingTop:12 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#1a1a2e', marginBottom:8 }}>⭐ 자주쓰는 품목 ({favorites.length}개)</div>
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            {favorites.map(f => (
              <div key={f.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 12px', background:'rgba(255,107,53,0.04)', borderRadius:9, border:'1px solid rgba(255,107,53,0.12)' }}>
                <div>
                  <span style={{ fontSize:12, fontWeight:600, color:'#1a1a2e' }}>{f.name}</span>
                  <span style={{ fontSize:10, color:'#aaa', marginLeft:8 }}>총 {f.use_count}회 · 최근 {numFmt(f.default_amount||0)}원</span>
                </div>
              ...
              <div key={entry.id} onClick={() => { setEditEntry(entry); setShowModal(true) }}
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  ...
                  <div style={{ textAlign:'right', marginLeft:12, flexShrink:0 }}>
                    <div style={{ fontSize:16, fontWeight:800, color:'#1a1a2e' }}>{numFmt(entry.amount)}원</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {showModal && (
        <EntryModal sheet={sheet} entry={editEntry} storeId={storeId} userName={userName}
          year={year} month={month} favorites={favorites}
...
function SalesView({ storeId, year, month }: { storeId: string; year: number; month: number }) {
  ...
  async function loadSales() {
    ...
    setDailySales(cls.map((cl:any) => {
      const platforms = (sv||[]).filter((s:any)=>s.closing_id===cl.id)
      const total = platforms.reduce((s:number,p:any)=>s+(p.amount||0),0)
      return { ...cl, total, platforms }
    }))
    setLoading(false)
  }
    ...
    <div>
      <div style={bx}>
        ...
        <div style={{ display:'flex', gap:12 }}>
          <span style={{ fontSize:11, color:'#bbb' }}>마감 {dailySales.length}일</span>
      ...
      {dailySales.length===0 ? (
        <div style={{ textAlign:'center', padding:'60px 0', color:'#bbb' }}>
          ...
          <div style={{ fontSize:12 }}>마감일지에서 매출 입력 시 자동 표시됩니다</div>
        </div>
      ) : dailySales.map(day => {
  ...
  async function loadAll() {
    ...
    await Promise.all(storeList.map(async (store:any) => {
      ...
      if (cls?.length) {
        ...
        sales = (sv||[]).reduce((s:number,r:any)=>s+(r.amount||0), 0)
      }
      const sums: Record<string,number> = {}
      ;(entries||[]).forEach((e:any) => { sums[e.sheet_id]=(sums[e.sheet_id]||0)+(e.amount||0) })
      ...
      result[sid] = {
        sales, expense,
        sheets: (sheets||[]).map((sh:any) => ({ ...sh, amount: sums[sh.id]||0 })),
  ...
  async function exportExcel() {
    try {
      ...
      stores.forEach((store:any) => {
        ...
        const rate = d.sales>0 ? Math.round((net/d.sales)*1000)/10 : 0
        ws.addRow([store.name, d.sales, d.expense, net, rate, d.settings?.business_type==='corporation'?'법인':'개인사업자'])
      })
      ...
      stores.forEach((store:any) => {
        ...
        d.sheets.forEach((sh:any) => {
          const r = d.sales>0 ? Math.round((sh.amount/d.sales)*1000)/10 : 0
          wsDetail.addRow([`${sh.icon} ${sh.name}`, sh.amount, r])
        })
        wsDetail.addRow(['💰 순수익', d.sales-d.expense, d.sales>0?Math.round(((d.sales-d.expense)/d.sales)*1000)/10:0])
        wsDetail.getColumn(2).numFmt='#,##0'
      ...
      a.href=url; a.download=`결산_${year}년${pad(month)}월.xlsx`; a.click()
  ...
  return (
    <div>
      <div style={{ ...bx, border:'1.5px solid rgba(108,92,231,0.3)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
          {[
            { l:'전체 매출', v:totSales, c:'#00B894', bg:'rgba(0,184,148,0.08)' },
            { l:'전체 지출', v:totExpense, c:'#E84393', bg:'rgba(232,67,147,0.06)' },
            { l:'전체 순수익', v:totSales-totExpense, c:(totSales-totExpense)>=0?'#00B894':'#E84393', bg:(totSales-totExpense)>=0?'rgba(0,184,148,0.08)':'rgba(232,67,147,0.06)' },
          ].map(item => (
            <div key={item.l} style={{ padding:'12px 8px', background:item.bg, borderRadius:12, textAlign:'center' }}>
              <div style={{ fontSize:10, color:'#aaa', marginBottom:3 }}>{item.l}</div>
              <div style={{ fontSize:13, fontWeight:800, color:item.c }}>{Math.abs(item.v)>=10000?`${(item.v/10000).toFixed(0)}만`:`${numFmt(item.v)}`}원</div>
            </div>
          ))}
      ...
      {stores.map((store:any) => {
        ...
        if (!d) return null
        const net = d.sales - d.expense
        const profR = d.sales>0 ? Math.round((net/d.sales)*1000)/10 : 0
        const bizLabel = d.settings?.business_type==='corporation'?'법인':'개인사업자'
        return (
          <div key={store.id} style={{ ...bx, border:`1.5px solid ${net>=0?'rgba(0,184,148,0.3)':'rgba(232,67,147,0.3)'}` }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              <div>
                ...
                <span style={{ fontSize:10, color:'#aaa', marginLeft:8, background:'#F4F6F9', padding:'2px 7px', borderRadius:5 }}>{bizLabel}</span>
              </div>
              <span style={{ fontSize:13, fontWeight:700, padding:'4px 12px', borderRadius:8, background: net>=0?'rgba(0,184,148,0.12)':'rgba(232,67,147,0.1)', color: net>=0?'#00B894':'#E84393' }}>
                수익률 {profR}%
              </span>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12 }}>
              {[
                { l:'매출', v:d.sales, c:'#00B894' },
                { l:'지출', v:d.expense, c:'#E84393' },
                { l:'순수익', v:net, c:net>=0?'#00B894':'#E84393' },
              ].map(item => (
                <div key={item.l} style={{ textAlign:'center', padding:'10px 6px', background:'#F8F9FB', borderRadius:10 }}>
                  <div style={{ fontSize:10, color:'#aaa' }}>{item.l}</div>
                  <div style={{ fontSize:14, fontWeight:800, color:item.c, marginTop:3 }}>
                    {Math.abs(item.v)>=10000?`${(item.v/10000).toFixed(0)}만`:`${numFmt(item.v)}`}원
                  </div>
                </div>
              ))}
            </div>
            {d.sheets.filter((sh:any)=>sh.amount>0).length>0 && (
              <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                {d.sheets.filter((sh:any)=>sh.amount>0).slice(0,6).map((sh:any) => (
                  <span key={sh.id} style={{ fontSize:10, background:'rgba(255,107,53,0.07)', color:'#FF6B35', padding:'2px 9px', borderRadius:10, fontWeight:600 }}>
                    {sh.icon} {sh.name} {numFmt(sh.amount)}원
                  </span>
                ))}
              </div>
            )}
            {d.sales===0 && d.expense===0 && (
              <div style={{ fontSize:11, color:'#bbb', textAlign:'center', padding:'8px 0' }}>결산 데이터 없음</div>
            )}
          </div>
        )
      })}
...
export default function SettlementPage() {
  ...
  useEffect(() => {
    ...
    if (user.role==='owner') {
      ...
      loadSheets(store.id); loadSettings(store.id)
  ...
  async function loadSettings(sid: string) {
    const { data } = await supabase.from('settlement_settings').select('*').eq('store_id', sid).maybeSingle()
    setSettings(data)
  }
    ...
    <div>
      ...
      <div style={{ overflowX:'auto', marginBottom:16, scrollbarWidth:'none' as const }}>
        <div style={{ display:'flex', gap:6, paddingBottom:4, minWidth:'max-content' }}>
          ...
          {isOwner && (
            <button onClick={() => setSelectedSheet('admin')}
              style={{ padding:'8px 14px', borderRadius:20, border: selectedSheet==='admin'?'2px solid #6C5CE7':'1px solid #E8ECF0', background: selectedSheet==='admin'?'rgba(108,92,231,0.1)':'#fff', color: selectedSheet==='admin'?'#6C5CE7':'#888', fontSize:12, fontWeight: selectedSheet==='admin'?700:500, cursor:'pointer', flexShrink:0 }}>
              👑 전지점
            </button>
          )}
        </div>
      ...
      {selectedSheet==='analysis' && (
        <ProfitAnalysisView sheets={activeSheets} storeId={storeId} year={year} month={month} settings={settings} />
      )}
      {selectedSheet==='sales' && (
        <SalesView storeId={storeId} year={year} month={month} />
      )}
      {selectedSheet==='admin' && isOwner && (
