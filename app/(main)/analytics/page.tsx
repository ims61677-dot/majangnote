...
export default function AnalyticsPage() {
  ...
  function renderTabContent() {
    if (loading) return <div style={{textAlign:'center',padding:'60px 0',color:'#aaa',fontSize:13}}>불러오는 중…</div>
        ...
        <div style={bx}>
          ...
          {daily.length>0 ? (
            <LineChart
              ...
              color="#FF6B35" height={isPC?260:160} />
          ) : (
        ...
        <div style={bx}>
          ...
          {daily.filter(d=>d.count>0).length>0 ? (
            <LineChart
              ...
              color="#6C5CE7" height={isPC?260:160} />
          ) : (
        ...
        <div style={bx}>
          ...
          {platforms.length>0 ? (
            <div style={{display:'flex',gap:24,alignItems:'center',flexWrap:'wrap'}}>
              <PieChart data={platforms.map(p=>({label:p.name,value:p.amount,color:p.color}))} size={isPC?180:130} />
  ...
  const topSection = (
    <>
      ...
      <div style={{display:'grid', gridTemplateColumns: isPC ? 'repeat(6,1fr)' : 'repeat(3,1fr)', gap:8, marginBottom:12}}>
        {summaryCards.map(s=>(
          <div key={s.label} style={{background:'#fff',borderRadius:12,border:'1px solid #E8ECF0',padding:isPC?'14px 18px':'10px 12px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
            <div style={{fontSize:10,color:'#aaa',marginBottom:3}}>{s.label}</div>
            <div style={{fontSize:isPC?18:14,fontWeight:800,color:s.color}}>{loading?'…':s.value}</div>
          </div>
        ))}
      </div>
  ...
  if (isPC) return (
    <div>
      ...
      {/* PC 레이아웃: 왼쪽 탭 세로 메뉴 + 오른쪽 콘텐츠 */}
      <div style={{display:'grid', gridTemplateColumns:'180px 1fr', gap:16, alignItems:'start'}}>
        {/* 왼쪽 세로 탭 */}
        <div style={{background:'#fff', borderRadius:16, border:'1px solid #E8ECF0', padding:8, boxShadow:'0 1px 4px rgba(0,0,0,0.04)', position:'sticky', top:80}}>
          {TAB_LIST.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id as any)}
              style={{
                display:'block', width:'100%', textAlign:'left',
                padding:'11px 14px', borderRadius:10, border:'none',
                background: tab===t.id ? 'linear-gradient(135deg,rgba(255,107,53,0.12),rgba(232,67,147,0.12))' : 'transparent',
                color: tab===t.id ? '#FF6B35' : '#888',
                fontSize:13, fontWeight: tab===t.id ? 700 : 500,
                cursor:'pointer', marginBottom:2,
                borderLeft: tab===t.id ? '3px solid #FF6B35' : '3px solid transparent',
