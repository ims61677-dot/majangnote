...
export default function GoalPage(){
  ...
  function addIssue(week:number){
    ...
    if(!text) return
    const newItem:Issue={ id: Date.now().toString(), text }
  ...
  function startEdit(week:number,issue:Issue){
    ...
    setEditingText(issue.text)
  }
  function saveEdit(week:number){
    if(!editingIssue) return
    setWeekIssues(p=>({
      ...p,
      [week]:(p[week]||[]).map(i=>i.id===editingIssue.id?{...i,text:editingText}:i)
    }))
    setEditingIssue(null); setEditingText('')
  }
        ...
        {monthGoalTotal>0&&(
          monthRemaining>0 ? (
            <div style={{borderRadius:12,padding:'12px 14px',
              background:monthRate<60?'rgba(232,67,147,0.08)':'rgba(255,107,53,0.07)',
              border:`1px dashed ${monthRate<60?'rgba(232,67,147,0.35)':'rgba(255,107,53,0.3)'}`}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}>
                <div>
                  ...
                  <div style={{fontSize:22,fontWeight:900,color:monthRate<60?'#E84393':'#FF6B35',lineHeight:1.1}}>
                    {fmtW(monthRemaining)}
                  </div>
                ...
                {isCurrentMonth&&monthRemainDays>0&&(
                  <div style={{textAlign:'right',background:'#fff',borderRadius:10,padding:'8px 12px',border:'1px solid #E8ECF0',flexShrink:0}}>
                    ...
                    <div style={{fontSize:16,fontWeight:800,color:monthRate<60?'#E84393':'#FF6B35'}}>
            ...
            <div style={{borderRadius:12,padding:'12px 14px',background:'rgba(0,184,148,0.08)',border:'1px solid rgba(0,184,148,0.25)',textAlign:'center'}}>
              ...
              <div style={{fontSize:11,color:'#00B894',marginTop:2}}>
                {fmtW(monthActualTotal-monthGoalTotal)} 초과 달성
              </div>
            </div>
          )
        )}
      ...
      {Array.from({length:totalWeeks},(_,i)=>i+1).map(week=>{
        ...
        return(
          <div key={week} style={{...bx,
            border:isCurrent&&rate<60&&calc.goal>0?'2px solid #E84393':isCurrent?'2px solid rgba(255,107,53,0.45)':'1px solid #E8ECF0'}}>
            ...
            <div onClick={()=>setExpandedWeek(isExpanded?null:week)} style={{cursor:'pointer'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  {issues.length>0&&(
                    <span style={{fontSize:10,background:'rgba(232,67,147,0.1)',color:'#E84393',padding:'2px 6px',borderRadius:6,fontWeight:700}}>
                      📌 {issues.length}건
                    </span>
                  )}
                ...
                <div style={{background:rate>=100?'rgba(0,184,148,0.07)':rate>=80?'rgba(255,107,53,0.07)':'rgba(232,67,147,0.07)',
                  ...
                  {weekRemaining>0&&(
                    <div style={{padding:'10px 12px',borderRadius:8,
                      background:rate<60?'rgba(232,67,147,0.08)':'rgba(255,107,53,0.06)',
                      border:`1px dashed ${rate<60?'rgba(232,67,147,0.3)':'rgba(255,107,53,0.25)'}`}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <div>
                          <div style={{fontSize:10,color:'#888',marginBottom:3}}>
                            🎯 달성까지 필요
                            {remainDays>0&&<span style={{color:'#bbb'}}> (남은 {remainDays}일)</span>}
                          </div>
                          <div style={{fontSize:18,fontWeight:900,color:rate<60?'#E84393':'#FF6B35',lineHeight:1.1}}>
                            {fmtW(weekRemaining)}
                          </div>
                        ...
                        {remainDays>0&&(
                          <div style={{textAlign:'right',background:'#fff',borderRadius:8,padding:'7px 10px',border:'1px solid #E8ECF0',flexShrink:0}}>
                            ...
                            <div style={{fontSize:14,fontWeight:800,color:rate<60?'#E84393':'#FF6B35'}}>
                              {fmtW(Math.ceil(weekRemaining/remainDays))}
                            </div>
                            <div style={{fontSize:9,color:'#bbb',marginTop:1}}>/일</div>
                          </div>
                        )}
                ...
                <div style={{marginBottom:16,background:'rgba(232,67,147,0.04)',borderRadius:12,padding:14,border:'1px solid rgba(232,67,147,0.15)'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
                    <div style={{display:'flex',alignItems:'center',gap:6}}>
                      ...
                      {issues.length>0&&(
                        <span style={{fontSize:10,background:'rgba(232,67,147,0.12)',color:'#E84393',padding:'2px 6px',borderRadius:10,fontWeight:700}}>
                          {issues.length}건
                        </span>
                      )}
                    </div>
                  ...
                  {issues.length===0&&(
                    <div style={{textAlign:'center',padding:'12px 0',fontSize:12,color:'#ccc'}}>
                  ...
                  {issues.map((issue,idx)=>(
                    <div key={issue.id} style={{marginBottom:8}}>
                      {editingIssue?.week===week&&editingIssue?.id===issue.id ? (
                        <div style={{background:'#fff',borderRadius:10,padding:10,border:'1px solid rgba(232,67,147,0.3)'}}>
                          <div style={{fontSize:10,color:'#E84393',fontWeight:600,marginBottom:6}}>✏️ {idx+1}번 수정 중</div>
                          <textarea
                            value={editingText}
                            onChange={e=>setEditingText(e.target.value)}
                            style={{...ta,minHeight:56,background:'#FFF5F8',borderColor:'rgba(232,67,147,0.25)',marginBottom:8}}
                            autoFocus
                          />
                          <div style={{display:'flex',gap:6}}>
                            <button onClick={()=>saveEdit(week)}
                              style={{flex:1,padding:'7px 0',borderRadius:8,border:'none',background:'#E84393',color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer'}}>
                              ✓ 저장
                            </button>
                            <button onClick={()=>{setEditingIssue(null);setEditingText('')}}
                              style={{flex:1,padding:'7px 0',borderRadius:8,border:'1px solid #E0E4E8',background:'#fff',color:'#888',fontSize:12,cursor:'pointer'}}>
                              취소
                            </button>
                          </div>
                        </div>
                        ...
                        <div style={{background:'#fff',borderRadius:10,padding:'10px 12px',border:'1px solid rgba(232,67,147,0.15)',
                          ...
                          <span style={{fontSize:11,color:'#E84393',fontWeight:700,flexShrink:0,marginTop:2}}>#{idx+1}</span>
                          <span style={{fontSize:12,color:'#1a1a2e',flex:1,lineHeight:1.6,whiteSpace:'pre-wrap'}}>{issue.text}</span>
                          {isOwner&&(
                            <div style={{display:'flex',gap:4,flexShrink:0}}>
                              ...
                              <button onClick={e=>{e.stopPropagation();if(confirm('이 지적사항을 삭제할까요?'))deleteIssue(week,issue.id)}}
                  ...
                  {isOwner&&(
                    <div style={{marginTop:8,display:'flex',gap:6,alignItems:'flex-end'}}>
                      <textarea
                        ...
                        placeholder="새 지적사항 입력... (Enter로 추가)"
                        style={{...ta,minHeight:44,flex:1,marginBottom:0,background:'#fff',borderColor:'rgba(232,67,147,0.2)'}}
                        onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();addIssue(week)}}}
                      />
                      <button onClick={()=>addIssue(week)}
                        style={{padding:'9px 14px',borderRadius:8,border:'none',background:'#E84393',color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer',flexShrink:0,height:44}}>
                        + 추가
                      </button>
                    </div>
                  )}
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
                  <div style={{background:'#FFF5F0',borderRadius:12,padding:12,border:'1px solid rgba(255,107,53,0.2)'}}>
                    <div style={{fontSize:10,color:'#FF6B35',fontWeight:700,marginBottom:6}}>
                      📅 평일 목표 <span style={{color:'#aaa',fontWeight:400}}>({calc.weekdays}일)</span>
                    </div>
                    {canEdit
                  ...
                  <div style={{background:'#FFF0F0',borderRadius:12,padding:12,border:'1px solid rgba(232,67,147,0.2)'}}>
                    <div style={{fontSize:10,color:'#E84393',fontWeight:700,marginBottom:6}}>
                      🔴 주말/공휴일 <span style={{color:'#aaa',fontWeight:400}}>({calc.redDays}일)</span>
                    </div>
                    {canEdit
